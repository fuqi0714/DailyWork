...
分析:
由题可知3个人共享5个盘子，并且盘子访问事件同一时刻只能满足1个人使用，因此在3个人不停对盘子进行操作的情况，在访问前需要对盘子的状态进行判断和验证。将盘子的数量、可使用盘子的用户以及水果种类量化为信号量
...
- 先判断是否有空闲的盘子
- 若有，对一个空盘子进行使用声明，避免在使用结束前儿子和女儿发出对该盘子的使用
...
儿女操作流程如下
- 先判断是否有满足要求的水鬼
- 若有，对一个非空盘子进行使用声明，避免在使用结束前被其他请求影响使用
    - 若无，则将流程暂停，等待满足使用情况继续使用
- 在取出满足要求的水果后，根据水果种类将对应的信号量减少。确保可用资源量化后可进行操作
